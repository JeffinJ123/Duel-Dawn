...
export class Game {
  ...
  freegameMessageTransition = new PIXI.Text("", {
    fill: 0xffffff,
    fontFamily: "Trebuchet MS",
    fontSize: 45,
    fontWeight: "bold",
  ...
  async start(): Promise<void> {
    const configResponse = await fetch(`${import.meta.env.BASE_URL}config/gameConfig.json`); // gameconfig loading for deployment
    ...
    // this.loadDummy();//dummy asset for testing
  }
  ...
  setupStageHierarchy(): void {
    ...
    this.hudLayer.position.set(0, 25);
    this.reelContainer.position.set(0, 30);
    ...
    this.stateMachine
      ...
      .addState({
        ...
        onEnter: () => {
          ...
          this.freegameMessageTransition.text = this.freeGameMessageText;
          this.freegameMessageTransition.position.set(
            this.config.app.width / 2,
            this.config.app.height / 2,
          );
          this.freegameMessageTransition.visible = true;
        },
        onExit: () => {
          this.freegameMessageTransition.visible = false;
        },
  ...
  createBackground(): void {
    const symbolTexture = PIXI.Assets.get("freeBG");
    // const symbolTexture = this.getSymbolTexture(label);
    const background = new PIXI.Sprite(symbolTexture);
    background.name = "background";
    background.anchor.set(0.5);
    background.scale.set(0.5);
    background.position.set(611,320);
    ...
    const title = new PIXI.Text("DUEL DAWN", {
      fill: 0xf6efe0,
      fontFamily: "Trebuchet MS",
      fontSize: 40,
      fontWeight: "bold",
  ...
  createTransitionText(): void {
    this.freegameMessageTransition.name = "freegameTransitionText";
    this.freegameMessageTransition.anchor.set(0.5);
    this.freegameMessageTransition.position.set(this.config.app.width / 2, 86);
    this.freegameMessageTransition.visible = false;
    this.winLayer.addChild(this.freegameMessageTransition);
  }
