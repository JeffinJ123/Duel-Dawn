...
export class SymbolFactory {
  ...
  createTexture(label: string): PIXI.RenderTexture {
    ...
    symbol.anchor.set(0);
    symbol.scale.set(0.2);
    // symbol.position.set(45, 25);
  ...
  getSymbolTexture(label: string): PIXI.Texture {
    console.log(label);
    const symbolTexture = PIXI.Assets.get("SymbolSheet");
    const texture = new PIXI.Texture(symbolTexture, new PIXI.Rectangle(200, 200, 600, 600));
    return texture;
  }
}
