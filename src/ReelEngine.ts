...
export class ReelEngine {
  ...
  showWins(result: SpinResult): void {
    ...
    console.log(result);
      ...
      for (let i = 0; i < this.reels.length; i++) {
        if(i > result.winLines[0].count - 1){
              return;
         }
            for (const entry of this.reels[i].sprites) {
              const resultRow = result.winLines as any;
              if(resultRow[0].lineIndex === entry.logicalRow ){
  ...
  createFrame(): void {
    // const totalWidth = this.config.reels * this.cellWidth + (this.config.reels - 1) * this.reelGap;

    // const frameSprite = new PIXI.Graphics();
    const Texture = PIXI.Assets.get("reelFrame");
    ...
    // frame.endFill();
    frame.scale.set(0.35,0.4);
    frame.position.set(300,206);
  ...
  createReels(): void {
    for (let reelIndex = 0; reelIndex < this.config.reels; reelIndex++) {
      ...
      reelContainer.x = reelIndex * (this.cellWidth + (this.reelGap + 5 ));
